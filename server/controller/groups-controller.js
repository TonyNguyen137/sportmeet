...
export const getGroupById = async (req, res) => {
	...
	try {
		const group = await findGroupForUser(groupId, userId);
		if (!group) {
			return res.status(404).render('base', {
				title: '404 - Seite nicht gefunden',
				template: 'page-404'
		...
		const members = await findGroupMembers(groupId, userId);
		const upcomingEvents = await findUpcomingEventsForGroup(groupId, userId, 12);
...
export const createGroup = async (req, res) => {
	...
	try {
		await createGroupWithAdmin(groupName, description, userId, createInviteCode, 5);
...
export const joinGroup = async (req, res) => {
	...
	try {
		const groupId = await findGroupIdByInviteCode(inviteCode);
		if (!groupId) {
			return res
				.status(404)
		...
		const membershipCount = await joinGroupById(groupId, userId);
...
export const joinGroupFromInviteLink = async (req, res) => {
	const userId = req.session?.userId;
	const inviteCode = extractInviteCode(req.query?.invite || req.query?.code || '');
...
export const deleteGroup = async (req, res) => {
	...
	try {
		const didDelete = await deleteGroupByIdForOwner(groupId, userId);
...
export const removeGroupMember = async (req, res) => {
	...
	try {
		const result = await removeGroupMemberByOwner(groupId, memberId, userId);
...
export const regenerateGroupInvite = async (req, res) => {
	const userId = req.session?.userId;
	const groupId = Number(req.params.groupId);
	...
	try {
		const didRegenerate = await regenerateInviteCodeByAdmin(
			groupId,
			userId,
			createInviteCode,
			5
