	...
	const result = await pool.query(
		`SELECT
		 ...
		 LEFT JOIN event_participants ep_self
		 	ON ep_self.event_id = e.id AND ep_self.user_id = $1
		 ...
		 WHERE e.start_datetime >= NOW()
		   AND (
				(
					...
					AND (
						e.created_by = $1
						OR ep_self.status = 'accepted'
					)
				)
				OR EXISTS (
					SELECT 1
					FROM group_users gu
					WHERE gu.group_id = e.group_id AND gu.user_id = $1
				)
			)
...
export const findCreatedEventsForUser = async (userId, limit = 100) => {
	const result = await pool.query(
		`SELECT
			e.id,
			e.title,
			...
			COALESCE(s.name, e.custom_sport_name) AS sport_name,
			u.first_name || ' ' || u.last_name AS creator_name,
			TRUE AS is_created_by_current_user,
			COALESCE(ep_stats.accepted_count, 0)::int AS accepted_count
		 FROM events e
		 LEFT JOIN groups g ON g.id = e.group_id
		 LEFT JOIN sports s ON s.id = e.sport_id
